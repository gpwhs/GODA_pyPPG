...
import pyPPG
from pyPPG.datahandling import load_data_from_dataframe
...
class PulseWaveAnal:
            ...
            try:
                if n == "sp":
                    exec(
                        "ref_"
                        + n
                        + " = np.array(np.squeeze(np.round(annot['annot']['pk'][0, 0][0][0]['t'] * fs).astype(int)))"
                    )
                elif n == "on":
                    ref_on = eval(
                        "np.array(np.squeeze(np.round(annot['annot']['os'][0, 0][0][0]['t'] * fs).astype(int)))"
                    )
                else:
                    exec(
                        "ref_"
                        + n
                        + " = np.array(np.squeeze(np.round(annot['annot']['"
                        + n
                        + "'][0, 0][0][0]['t'] * fs).astype(int)))"
                    )
    ...
    def print_error(self, d_error, lable1, annot2, numel):
        ...
        print(
            atype, " " + str(numel + 1) + "/219" + ", ID: ", self.name, "\ndifferences:"
        )
    ...
    def save_all_data(
        ...
        self.OutData[annot1 + "_fps"] = (
            self.M_FID_1
        )  # fiducial points of the 1st annotation
        if save:
            if compare:
                tmp_dir = (
                    "results" + os.sep + dname + os.sep + annot1 + "_" + annot2 + os.sep
                )
                os.makedirs(tmp_dir, exist_ok=True)

                # self.OutData[annot2+'_ae'] = annot_error2   # 2nd annotation error (0,1)
                self.OutData[annot2 + "_fps"] = (
                    self.M_FID_2
                )  # fiducial points of the 2nd annotation
                self.OutData[annot1 + "_" + annot2 + "_diff"] = (
                    dist_error  # annotation difference (sample)
                )
                mat_file = tmp_dir + annot1 + "_" + annot2 + ".mat"

            else:
                self.OutData[annot1 + "_pyPPG_diff"] = (
                    dist_error  # detection difference (sample)
                )
            ...
            matlab_data = {
                key: value.to_records(index=False)
    ...
    def get_validation(
        ...
        try:
            ppg_fp["on"] = (
                np.argmax(s.jpg[int(apg_fp.a[0]) - win_on : int(apg_fp.a[0])])
                + apg_fp.a[0]
                - win_on
            )
        except:
        ...
        ppg_fp["sp"] = (
            np.argmax(s.ppg[int(ref_fp.on[0]) : int(ref_fp.off[0])]) + ref_fp.on[0]
        )
    ...
    ###########################################################################
    def get_stats(
        self, fp_names, dist_error, ppg_IDs, dname, params, filename, detector_dir, prnt
    ):
        ...
        for n in fp_names:
            exec(
                "MAE['"
                + n
                + "'] = np.round(np.nanmean(np.absolute(dist_error."
                + n
                + ")),2)"
            )
        ...
        dist_error.index = dist_error.index.map(
            lambda x: ppg_IDs[x - 3] if isinstance(x, int) else x
        )
    ...
    def run_ppg_bp_eval(
        ...
        for i in range(0, set_len):
            ...
            if compare:
                ref2_fp, annot_diff, annot_err = self.get_annot_diff(
                    s,
                    ref_fp,
                    self.annot_path2,
                    self.name,
                ...
                det_fp, annot_diff = self.get_validation(
                    s=s,
                    ref_fp=ref_fp,
                    plt_sig=plt_sig,
            ...
            # Print distance error
            if prnt_e:
                self.print_error(annot_diff, lable1, lable2, i)
        ...
        if compare:
            ...
            corr_txt = "\ncorrection: " + str(correction.to_dict()).replace(
                "{0:", ""
            ).replace("},", ",").replace("}}", "}")
        ...
        self.get_stats(
            fp_names, dist_error, ppg_IDs, dname, params, filename, detector_dir, prnt
        )
    ...
    def benchmark_PPG_BP(self, detector, dname, plt, prnt):
        ...
        for annotator in ["MG", "PC"]:
            ...
            dist_error = pd.DataFrame(
                flat_diffs.tolist(), columns=raw_diffs.dtype.names
            )
            ...
            if plt:
                ...
                for ID in ppg_IDs:
                    ...
                    ref_fp = pd.DataFrame(
                        [
                            {
                                key: np.NaN if pd.isna(value) else value
                    ...
                    if plt_sig:
                        self.plot_pulse_wave(
                            ...
                            fp1=ref_fp,
                            fp2=det_fp,
                            d_error=dist_error.iloc[n].to_dict(),
                            compare=False,
                            show_fig=False,
                            dname=dname,
                            annot1=annotator,
            ...
            self.get_stats(
                fp_names, dist_error, ppg_IDs, dname, params, name, detector_dir, prnt
            )

            tmp_dir = "results" + os.sep + dname + os.sep + detector + os.sep
            csv_file1 = (
                tmp_dir + annotator + "_" + detector + os.sep + annotator + "_fps.csv"
            )
            csv_file2 = (
                tmp_dir + annotator + "_" + detector + os.sep + detector + "_fps.csv"
            )
    ...
    def get_mean_results(self, tmp_dir, detector):
        MG_diff_path = (
            tmp_dir + "MG_" + detector + os.sep + "MG_" + detector + "_diffs.csv"
        )
        PC_diff_path = (
            tmp_dir + "PC_" + detector + os.sep + "PC_" + detector + "_diffs.csv"
        )
    ...
    ######################## Extract Pulse Wave Features  #####################
    ###########################################################################
    def run_pw_extraction(self, correction):
        savingfolder = "temp_dir" + os.sep + "PW_anal_01"
        datafolder = "Single_PW"
        self.extract_pw_feat(
            datafolder, savingfolder, correction, savefig=False, show_fig=True
        )
    ...
    def run_matlab_script(self, script_folder, script, param1, param2):
        if param2 == "":
            command = [
                "matlab",
                "-batch",
                f"eval('cd(''{script_folder}''); {script}(''{param1}'')')",
            ]
        else:
            command = [
                "matlab",
                "-batch",
                f"eval('cd(''{script_folder}''); {script}(''{param1}'',''{param2}'')')",
            ]
