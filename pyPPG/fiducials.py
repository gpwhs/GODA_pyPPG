...
class FpCollection:
    ...
    ###########################################################################
    def detect_maxima(
        self, sig: np.array, percentile: int, hr_win: int, peak_detector: str
    ):
        # Table VI pseudocode
        """
    ...
    ###########################################################################
    def def_bandpass(
        self, sig: np.array, fs: int, lower_cutoff: float, upper_cutoff: float
    ):
        """
    ...
    ###########################################################################
    def find_onsets(
        self, sig: np.array, fs: int, up: DotMap, peaks: np.array, med_hr: float
    ):
        """
                ...
                try:
                    ...
                    if len(min_dn) > 0 and diff_dn > round(self.fs / 100):
                        ...
                        try:
                            ...
                            fiducials.dn[i] = (
                                find_peaks(-self.ppg[strt_dn:stp_dn])[0][-1] + strt_dn
                            )
            ...
            if correction.v[0] and correction.w[0]:
                if fiducials.v[i] > fiducials.e[i]:
                    try:
                        fiducials.loc[i, "v"] = [
                            np.argmin(
                                self.vpg[int(fiducials.u[i]) : int(fiducials.e[i])]
                            )
                            + fiducials.u[i]
                        ]
                        fiducials.loc[i, "w"] = [
